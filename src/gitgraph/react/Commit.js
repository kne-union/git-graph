...
export const Commit = props => {
  const { commit, commits, gitgraph, commitMessagesX, currentCommitOver, setCurrentCommitOver, setTooltip, getWithCommitOffset } = props;
  ...
  const arrows = React.useMemo(() => {
    ...
    return commit.parents.map(parentHash => <Arrow key={parentHash} commits={commits} commit={commit} gitgraph={gitgraph} parentHash={parentHash} commitRadius={commitRadius} />);
  ...
  const branchLabels = React.useMemo(() => {
    ...
    return branches.map(branch => <BranchLabel key={branch.name} gitgraph={gitgraph} branch={branch} commit={commit} ref={branchLabelRef} branchLabelX={branchLabelX} />);
  ...
  const tags = React.useMemo(() => {
    ...
    return commit.tags.map((tag, i) => <Tag key={commit.hashAbbrev + '-' + tag.name} commit={commit} tag={tag} ref={r => (tagRefs.current[i] = r)} tagX={tagXs[i] || 0} />);
  ...
  const shouldRenderTooltip = currentCommitOver === commit && (gitgraph.isHorizontal || (gitgraph.mode === Mode.Compact && commit.style.hasTooltipInCompactMode));
  ...
  return (
    <g transform={`translate(${x}, ${y})`}>
      ...
      <g transform={`translate(${-x}, 0)`}>
        {commit.style.message.display && <Message commit={commit} ref={messageRef} messageX={messageX} />}
        {branchLabels}
        {tags}
      </g>
    </g>
