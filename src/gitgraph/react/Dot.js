...
export const Dot = ({ commit, onMouseOver, onMouseOut }) => {
  ...
  return (
    ...
    */
    <React.Fragment>
      <defs>
        <circle id={commit.hash} cx={commit.style.dot.size} cy={commit.style.dot.size} r={commit.style.dot.size} fill={commit.style.dot.color} />
      ...
      <g onClick={commit.onClick} onMouseOver={onMouseOver} onMouseOut={onMouseOut}>
        <use xlinkHref={`#${commit.hash}`} clipPath={`url(#clip-${commit.hash})`} stroke={commit.style.dot.strokeColor} strokeWidth={commit.style.dot.strokeWidth && commit.style.dot.strokeWidth * 2} />
        {commit.dotText && (
          <text alignmentBaseline="central" textAnchor="middle" x={commit.style.dot.size} y={commit.style.dot.size} style={{ font: commit.style.dot.font }}>
            {commit.dotText}
          </text>
        )}
      </g>
