...
function DefaultBranchLabel({ branch, commit }) {
  ...
  return (
    <g>
      <rect stroke={branch.style.label.strokeColor || commit.style.color} fill={branch.style.label.bgColor} rx={branch.style.label.borderRadius} width={boxWidth} height={boxHeight} />
      <text ref={getSizing} fill={branch.style.label.color || commit.style.color} style={{ font: branch.style.label.font }} alignmentBaseline="middle" dominantBaseline="middle" x={BranchLabel.paddingX} y={boxHeight / 2}>
        {branch.name}
      </text>
    </g>
  );
...
export const BranchLabel = React.forwardRef((props, ref) => {
  ...
  const branchLabel = branch.renderLabel ? branch.renderLabel(branch) : <DefaultBranchLabel branch={branch} commit={commit} />;
