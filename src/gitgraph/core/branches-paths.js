...
export class BranchesPathsCalculator {
  ...
  withMergeCommits() {
    ...
    mergeCommits.forEach(mergeCommit => {
      ...
      this.branchesPaths.set(branch, [...lastPoints, { x: mergeCommit.x, y: mergeCommit.y, mergeCommit: true }]);
    });
  }
 ...
 * @param {Array<Array<{x: number, y: number}>>} coordinates Collection of coordinates
 * @param {boolean} isBezier Use bezier curves
 * @param {boolean} isVertical Graph is vertical
 * @returns {string}
 */
export function toSvgPath(coordinates, isBezier, isVertical) {
  return coordinates
    .map(
      path =>
        'M' +
