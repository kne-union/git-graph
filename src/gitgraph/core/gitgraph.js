...
export class GitgraphCore {
  constructor(options = {}) {
    ...
    // Initialize state FIRST (before creating branches)
    this.refs = new Refs();
    this.tags = new Refs();
    this.tagStyles = {};
    this.tagRenders = {};
  ...
  get isHorizontal() {
    return this.orientation === Orientation.Horizontal || this.orientation === Orientation.HorizontalReverse;
  }
  ...
  get isReverse() {
    return this.orientation === Orientation.HorizontalReverse || this.orientation === Orientation.VerticalReverse;
  }
   ...
   *
   * @param {Function} listener A callback to be invoked on every change.
  ...
  /**
   * Create a new branch.
   * @param {Object|string} args Options of the branch or name
   */
  createBranch(args) {
  ...
  computeRenderedCommits() {
    ...
    const reachableUnassociatedCommits = (() => {
      ...
      const tipsOfMergedBranches = this.commits.reduce(
        (tipsOfMergedBranches, commit) => (commit.parents.length > 1 ? [...tipsOfMergedBranches, ...commit.parents.slice(1).map(parentHash => this.commits.find(({ hash }) => parentHash === hash))] : tipsOfMergedBranches),
        []
      );
      ...
      tipsOfMergedBranches.forEach(tip => {
        ...
        while (currentCommit && unassociatedCommits.has(currentCommit.hash)) {
          ...
          currentCommit = currentCommit.parents.length > 0 ? this.commits.find(({ hash }) => currentCommit.parents[0] === hash) : undefined;
        }
      });
    ...
    return (
      commitsWithBranches
        ...
        .map(commit =>
          commit.setTags(
            this.tags,
            name => Object.assign({}, this.tagStyles[name], this.template.tag),
   ...
   * @param {Array} commits List of commits with rendering data computed
   */
   ...
   * @param {Array} commits List of graph commits
   * @param {Map} branchesPaths Branches paths to be rendered
   */
   ...
   * @param {Map} branches All branches mapped by commit hash
   ...
   * @param {BranchesOrder} branchesOrder Computed order of branches
   * @param {Commit} commit Commit to position
   */
